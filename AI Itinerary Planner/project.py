...
def generate_itinerary(city: str, interests: List[str], days: int) -> str:
    ...
    try:
        ...
        return (
            response.text
            if response
            else "⚠️ Could not generate itinerary. Please try again."
        )
...
def create_ui():
    def get_itinerary():
        ...
        try:
            ...
            messagebox.showerror(
                "Input Error",
                "Please enter a valid city, interests, and number of days.",
            )
            return
        ...
        add_colored_text(
            result_text, f"🌍 {days}-Day Travel Plan for {city} 🌍\n", "title"
        )
        ...
        for line in itinerary.split("\n"):
            if "📅 Day" in line:
                ...
                result_text.insert(
                    tk.END, line.replace("🌅 Morning:", "").strip() + "\n"
                )
            elif "🌞 Afternoon:" in line:
                add_colored_text(result_text, "🌞 Afternoon: ", "afternoon")
                result_text.insert(
                    tk.END, line.replace("🌞 Afternoon:", "").strip() + "\n"
                )
            elif "🌙 Evening:" in line:
                add_colored_text(result_text, "🌙 Evening: ", "evening")
                result_text.insert(
                    tk.END, line.replace("🌙 Evening:", "").strip() + "\n"
                )
            elif (
                "🌦️ Weather Conditions:" in line or "🚗 Best Mode of Transport:" in line
            ):
                add_colored_text(result_text, f"\n{line}\n", "section_title")
            else:
    ...
    tk.Label(
        root, text="Enter City:", font=("Arial", 14, "bold"), bg="#f0f8ff", fg="#333"
    ).pack(pady=5)
    ...
    tk.Label(
        root,
        text="Enter Interests (comma separated):",
        font=("Arial", 14, "bold"),
        bg="#f0f8ff",
        fg="#333",
    ).pack(pady=5)
    ...
    result_text.tag_configure(
        "day_title", font=("Arial", 14, "bold"), foreground="#228B22"
    )
    result_text.tag_configure(
        "morning", font=("Arial", 12, "bold"), foreground="#ff4500"
    )
    result_text.tag_configure(
        "afternoon", font=("Arial", 12, "bold"), foreground="#D2691E"
    )
    result_text.tag_configure(
        "evening", font=("Arial", 12, "bold"), foreground="#800080"
    )
    result_text.tag_configure(
        "section_title", font=("Arial", 14, "bold"), foreground="#0047AB"
    )
